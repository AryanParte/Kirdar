...
import { Send, User, MessageSquare, Bot, Loader, Brain } from 'lucide-react';
...
const Simulation = ({ isGuest = false, initialData = null, onEnd = null }) => {
  ...
  // Initialize mentor state based on guest features
  ...
  console.log('Simulation Features:', { canUseMentor, canUseEvaluator, guestFeatures });
  ...
  const handleSubmit = async (e, directMessage = null) => {
    ...
    try {
      ...
      const endpoint = isGuest ? 'http://localhost:5001/api/guest/chat' : 'http://localhost:5001/api/chat';
      
      const headers = {
        'Content-Type': 'application/json',
        ...(token && { 'Authorization': `Bearer ${token}` }),
        ...(isGuest && { 'guestCode': guestCode })
      };
      ...
      // Prepare context data based on simulation type
      let contextData;
      if (simulationData.type === 'persona') {
        contextData = {
